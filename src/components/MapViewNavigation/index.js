...
export default class MapViewNavigation extends Component {
  ...
  getZoomValue(level) {
    const value =
      0.00001 *
      (this.props.maxZoom -
        (level < this.props.minZoom ? this.props.minZoom : level));
  ...
  prepareRoute(origin, destination, options = false, testForRoute = false) {
    ...
    options = Object.assign(
      ...
      options.constructor == Object ? options : {}
    );

    return this.directionsCoder
      .fetch(origin, destination, options)
      .then((routes) => {
        if (routes.length) {
  ...
  getPositionMarker(position, navigationMode) {
    const type =
      navigationMode == NavigationModes.NAVIGATION ? POSITION_ARROW : undefined;
  ...
  getRoutePolylines(route) {
    ...
    return route.polylines.map((params, index) => {
      return params ? (
        <RoutePolyline key={index} theme={this.props.theme} {...params} />
      ) : null;
    });
  }
  ...
  getDebugShapes(route) {
    ...
    steps.forEach((step, index) => {
      ...
      [
        ...
        {
          radius:
            this.props.routeStepDistance * this.props.routeStepInnerTolerance,
          color: "red",
        },
        {
          radius:
            this.props.routeStepDistance * this.props.routeStepCenterTolerance,
          color: "green",
        },
      ].forEach((d) => {
        result.push(
